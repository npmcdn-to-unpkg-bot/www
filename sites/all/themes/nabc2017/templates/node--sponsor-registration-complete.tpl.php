...
<?php
 global $base_url;
 validate_non_paid_sponsor_member_session();
...
<?php print getFormErrors();?>         
                ...
                <div class="heading-registration"><h4><span class="blacktext">Sponsor Registration to</span> #iNDULGE <span class="blacktext">@ NABC2017</span></h4></div>
                    <div class="col-sm-12 text-right"><?php /*<a href="<?php print url('book-hotel'); ?>">Book Hotel &nbsp;<i class="fa fa-bed"></i></a>*/?> <a href="<?php print url('sponsor/logout'); ?>">Logout &nbsp;<i class="fa fa-lock"></i></a></div>
...
var sponsor_badge_incrementor = <?php if($_SESSION['no_of_pre_fillled_sponsor_badge'] > 0) { print $_SESSION['no_of_pre_fillled_sponsor_badge']+1; } else { print '2'; }?>;
...
function reg_goto(step){
        ...
        if(jQuery('#core-overrides-membership-registration-sponsor-saved-form').valid()){
...
function changeStateForm(){
     if(jQuery('#field_sponsor_country').val() == 'US'){
          ...
          jQuery('.form-item-field-sponsor-state').parent().removeClass('hide-fields'); 
          ...
          jQuery('.form-item-field-sponsor-state-others').parent().removeClass('hide-fields');  
...
function update_total_price(){
    ...
    /***********************************/
        ...
        if(membership_plan == '137'){
			...
			no_of_allowed_badge_names = 5; // 5 Adult + Child
			
		}
		
		if(membership_plan == '136'){
            $membership_price = 20000.00; // Emerald Plan
			no_of_allowed_badge_names = 3; // 3 Adults
        ...
        if(membership_plan == '133'){
			...
			no_of_allowed_badge_names = 1; // 1 Adults
...
function add_remove_element_on_sponsor_badge_name(){
    for(i=<?php if($_SESSION['no_of_pre_fillled_sponsor_badge'] > 0) { print $_SESSION['no_of_pre_fillled_sponsor_badge']+1; } else { print '2'; }?>; i<=no_of_allowed_badge_names; i++){
...
jQuery(document).ready(function(event){
		...
		jQuery("#core-overrides-membership-registration-sponsor-saved-form").on("submit", function(){
            update_total_price();
         });
        
        jQuery("#core-overrides-membership-registration-sponsor-saved-form").validate({
                    errorElement: "em",
